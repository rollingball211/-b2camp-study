...
public class DispatcherServlet extends HttpServlet {
    ...
    try {
      ...
      for (File f : controllerFiles) {
        ...
        if (anno != null) {
          ...
          //2) 생성자를 알아냈다면 ,생성자의 파라미터 정보를 알아냄
          Parameter[] parameters = constructor.getParameters();

          //3 생성자를 호출하기 전에 파라미터 타입에 해당하는 아규먼트를 준비
          //- 아규먼트를 담을 배열 준비.
          Object[] arguments = new Object[parameters.length];


          //파라미터 타입에 일치하는 값을 beanContainer에서 찾음.
          outer: for(int i =0; i < parameters.length; i++) {

            for (Object value : beanContainer.values()) {
              if (parameters[i].getType().isInstance(value)) {
                //beanContainer에 들어 있는 값의 타입이 파라미터 타입과 일치하는 값을 찾앗으면
                //아규먼트 배열에 저장한다.
                arguments[i] = value;
                continue outer;
              }
          ...
          //4) 준비된 아규먼트들을 이용하여 생성자를 호출.
          PageController controller = (PageController) constructor.newInstance(arguments);

          //페이지 컨트롤러의 요청 URL 정보를 추출함.
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    PageController controller = controllerMap.get(controllerPath);
    if (controller == null) {
    ...
    try {
      Method[] methods = controller.getClass().getMethod();
